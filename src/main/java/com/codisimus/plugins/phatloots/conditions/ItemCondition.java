...
public class ItemCondition extends LootCondition {
    ...
    private String formattedItemName() {
        ...
        String plainName = PhatLootsUtil.getItemName(this.item);
        if (this.item.getItemMeta() == null || !this.item.getItemMeta().hasDisplayName()) {
            return plainName + " x" + this.item.getAmount();
        }
        return this.item.getItemMeta().getDisplayName() + " §r§e(" + plainName + ") x" + this.item.getAmount();
    }
